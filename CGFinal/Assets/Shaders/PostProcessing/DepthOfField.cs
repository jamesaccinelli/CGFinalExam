...
{
    const int circleOfConfusionPass = 0; // determine how we blit
    const int preFilterPass = 1;
    const int bokehPass = 2;
    const int postFilterPass = 3;
    const int combinePass = 4;

    public Shader dofShader; // let us add shader to camera
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination) // apply postprocess after cam renders
    {
        ...
        Graphics.Blit(source, coc, dofMaterial, circleOfConfusionPass); // blit in order of passes
